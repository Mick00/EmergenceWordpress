...
$post = $recent_posts[0];
?>
<div class="emergence-articles d-flex flex-sm-wrap flex-md-nowrap mt-4">
  <div class="article-principal mb-md-4 mb-sm-4 wow slideInLeft" style="background-image: url('<?=wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' )[0];?>')">
  ...
  <?php for ($i = 1; $i < count($recent_posts); $i++):
    $post = $recent_posts[$i];
    ?>
    <div class="article mb-md-4 ml-md-4 mb-sm-4 wow slideInRight" style="background-image: url('<?=wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'single-post-thumbnail' )[0];?>')">
      <div class="overlay d-flex">
        <div class="article-meta d-flex">
          <div class="article-date text-center bg-light">
            <span class="font-weight-bold text-secondary d-block"><?=date('d',$date)?></span>
